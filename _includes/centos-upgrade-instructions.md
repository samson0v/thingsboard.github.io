...
{% if docsPrefix == "pe/" %}
```bash
wget https://dist.thingsboard.io/thingsboard-{{ current_version_with_platform }}.rpm
```
{: .copy-code}
{% else %}
```bash
wget https://github.com/thingsboard/thingsboard/releases/download/v{{ current_version_with_platform }}/thingsboard-{{ current_version_with_platform }}.rpm
```
{: .copy-code}
{% endif %}
...
{% if docsPrefix == "pe/" %}
* Install Thingsboard Web Report component as described [here](/docs/user-guide/install/pe/rhel/#step-9-install-thingsboard-webreport-component).
{% endif %}

```bash
sudo rpm -Uvh thingsboard-{{ current_version_with_platform }}.rpm
```
{: .copy-code}
...
In case action is required, you can refer to *"you will need to install Java..."* section of [**Cassandra installation guide**](/docs/user-guide/install/rhel/?rhelThingsboardDatabase=hybrid#cassandra-installation).
{% endcapture %}
